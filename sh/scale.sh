...
if (($h > 768)); then
	echo "Height: $h, Scaling..."
	/usr/bin/ffmpeg -y -i "$1" -vf "scale=-2:768" -movflags faststart -preset veryfast -crf 24 -c:v libx264 -c:a copy "$1-hs.mp4" && /usr/bin/mv -f "$1-hs.mp4" "$1" && echo "Done - selesai - OK"
else
	echo "Height: $h, No scaling"
fi
exit
